...
| `speechbrain/spkrec-ecapa-voxceleb` | `core/speaker_auth.py` | no; downloaded at runtime | exact model id and private cache are regression-tested without downloading weights; synthetic enrollment tests cover malformed/non-finite rejection and owner-only file permissions; model-card license, training-data limits, cache/checksum, and biometric disclosure must still be reviewed |
