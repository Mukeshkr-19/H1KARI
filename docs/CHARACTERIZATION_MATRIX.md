...
| Voice recognition backends | no-load CLI status for installed packages, exact model ids, expected cache paths, offline readiness, explicit backend selection, and Google audio egress | high | Resolved in `feature/voice-backend-status`. Model weights still require provenance/checksum records before bundling or release claims. | blocking regression gate |
