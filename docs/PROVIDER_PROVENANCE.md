...
| Google Gemini Developer API | `gemini-3.5-flash-lite` for fast requests; `gemini-3.6-flash` for balanced/smart requests (overridable through explicit `GOOGLE_*_MODEL` settings) | system instructions, context, user text, generation settings | [paid-service data controls and limited abuse logging](https://ai.google.dev/gemini-api/docs/zdr); free and paid terms differ | unset `GOOGLE_AI_STUDIO_KEY`; select another configured provider |
